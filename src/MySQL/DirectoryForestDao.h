...
class DirectoryForestDao {
 public:
  ...
  // 删除文件
  bool delFile(const std::string& userId,const DirectoryForest& dirForest);
  // 文件文件夹
  bool delDir(const std::string& userId,const DirectoryForest& dirForest);
  // 添加文件
  ...
  // 获取父目录ID
  int getFatId(const std::string& path);
